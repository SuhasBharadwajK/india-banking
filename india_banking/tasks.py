...
from india_banking.india_banking.doctype.banking_backend_connector.banking_backend_connector import (
	get_banking_backend_connector,
	get_payment_status,
)
...
def process_batch_payment(payment_details):
	...
	for payment_order, summary in payment_details.items():
		...
		last_call = None
		payment_delay = banking_backend_connector.payment_call_interval or 10 # default 10 sec
		for summary_row in payment_order_doc.summary:
			if not last_call:
				last_call = time.time()
			else:
				last_call = banking_backend_connector.check_payment_delay(last_call)
			...
			if summary_row.name in summary:
				try:
					...
					banking_backend_connector.action = "initiate_payment"
					banking_backend_connector.make_single_request(payment_order_doc, summary_doc)
					enqueue_count += 1
				except Exception:
					frappe.log_error(
