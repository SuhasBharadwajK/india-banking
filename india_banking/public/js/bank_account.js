  ...
  refresh(frm) {
    if (frm.doc.is_company_account && !frm.doc.disabled) {
      frm.add_custom_button(__("Fetch Balance"), function () {
        frappe.call({
          method:
            "india_banking.india_banking.doctype.banking_backend_connector.banking_backend_connector.get_bank_balance",
          freeze: true,
          args: {
      ...
      frm.add_custom_button(__("Fetch Statements"), function () {
        const dialog = new frappe.ui.Dialog({
          ...
          primary_action: () => {
            frm.call({
              method:
                "india_banking.india_banking.doctype.banking_backend_connector.banking_backend_connector.get_bank_statements",
              args: {
